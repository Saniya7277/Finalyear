...
- **Scan before encrypt, encrypt before upload, upload before record.** The client performs the scan and AES encryption locally, uploads ciphertext directly to Supabase with a signed URL, then records metadata through `POST /api/files/complete-upload`. The API never accepts file bytes.
